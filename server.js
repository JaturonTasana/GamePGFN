...
io.on('connection', function(socket) {
	...
	socket.on('play', function(data) {
		...
		if(clients.length === 0) {
			...
			numberOfcamare = data.camareSpawnPoints.length;
			camareSpawnPoints = [];
			data.camareSpawnPoints.forEach(function(camareSpawnPoints) {
				var camareSpawnPoint = {
					position: camareSpawnPoints.position,
					rotation: camareSpawnPoints.rotation
				};
				enemies.push(camareSpawnPoint);
			});
/*
			camareSpawnPoints = [];
			data.camareSpawnPoints.forEach(function(_camareSpawnPoints) {
				var camareSpawnPoint = {
					position: _camareSpawnPoints.position,
					rotation: _camareSpawnPoints.rotation
				};
				camareSpawnPoints.push(camareSpawnPoint);
			});*/
		}
		...
		socket.emit('camareplay', currentPlayer);
...
/*
	socket.on('camareplay', function(data) {
			camareSpawnPoints = [];
			data.camareSpawnPoints.forEach(function(_camareSpawnPoints) {
				var camareSpawnPoints = {
					position: _camareSpawnPoints.position,
				...
				camareSpawnPoints.push(camareSpawnPoints);
			});

		var randomSpawnPoint = camareSpawnPoints[Math.floor(Math.random() * camareSpawnPoints.length)];
		currentPlayer = {
			name:data.name,
			position: randomSpawnPoint.position,
			rotation: randomSpawnPoint.rotation,
		...
		clients.push(currentPlayer);
		// in your current game, tell you that you have joined
		console.log(currentPlayer.name+' camare '+JSON.stringify(currentPlayer));
		socket.emit('camareplay', currentPlayer);
		// in your current game, we need to tell the other players about you.
		socket.broadcast.emit('other player connected', currentPlayer);
	}); */
	...
	socket.on('disconnect', function() {
		console.log(currentPlayer.name+' disconnect ');
		socket.broadcast.emit('other player disconnected', currentPlayer);
		//console.log(currentPlayer.name+' other player disconnected '+JSON.stringify(currentPlayer));
		for(var i=0; i<clients.length; i++) {
